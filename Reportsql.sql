...
SELECT k.companyId AS cid,ck.periodId AS pid,SUM(ck.claPoint) AS value, 'Sum_ClaPoint' As type
FROM pdca_kpi k 
...
SELECT k.companyId AS cid,ck.periodId AS pid,ck.achivement AS value, 'Trading_Profit' As type
FROM pdca_kpi k 
INNER JOIN pdca_cla_kpi ck
ON k.kpiId = ck.kpiId
...
SELECT k.companyId AS cid,ck.periodId AS pid,ck.ol AS value, 'Trading_Profit' As type
FROM pdca_kpi k 
INNER JOIN pdca_cla_kpi ck
ON k.kpiId = ck.kpiId
WHERE k.year = 2017 AND ck.periodId IN (3,4,5) AND k.itemDescriptionName = 1
